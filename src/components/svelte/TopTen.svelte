...
<div class="font-mono">
  ...
  {:else if data}
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      <section class="fade-card bg-[var(--color-eve-dark)] border border-[var(--color-eve-border)] rounded-sm overflow-hidden">
        <header class="bg-black/40 px-3 py-2 border-b border-[var(--color-eve-border)] text-xs tracking-widest text-[var(--color-isk-billion)] uppercase">TOP FINAL-BLOW WEAPONS</header>
        {@render rankList(data.finalBlowWeapon ?? [], (it) => itemName(it.id))}
      </section>

      <section class="fade-card bg-[var(--color-eve-dark)] border border-[var(--color-eve-border)] rounded-sm overflow-hidden">
        <header class="bg-black/40 px-3 py-2 border-b border-[var(--color-eve-border)] text-xs tracking-widest text-[var(--color-isk-billion)] uppercase">TOP KILLING SHIPS</header>
        {@render rankList(data.killerShip ?? [], (it) => shipName(it.id))}
      </section>

      <section class="fade-card bg-[var(--color-eve-dark)] border border-[var(--color-eve-border)] rounded-sm overflow-hidden">
        <header class="bg-black/40 px-3 py-2 border-b border-[var(--color-eve-border)] text-xs tracking-widest text-[var(--color-isk-billion)] uppercase">TOP KILLER CORPS</header>
        {@render rankList(data.killerCorp ?? [], (it) => it.name ?? `Corp ${it.id}`)}
      </section>

      <section class="fade-card bg-[var(--color-eve-dark)] border border-[var(--color-eve-border)] rounded-sm overflow-hidden">
        <header class="bg-black/40 px-3 py-2 border-b border-[var(--color-eve-border)] text-xs tracking-widest text-[var(--color-isk-billion)] uppercase">TOP KILLER ALLIANCES</header>
        {@render rankList(data.killerAlliance ?? [], (it) => it.name ?? `Alliance ${it.id}`)}
      </section>

    </div>

    <p class="text-[10px] tracking-widest text-white/20 uppercase mt-4 text-center">
      Rolling 1h window · final-blow based · NPC kills excluded from killer boards
    </p>
  {/if}
</div>
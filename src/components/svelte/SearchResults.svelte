...
{#each $searchResults?.kills ?? [] as k (k.killID)}
      <li class="border-b border-[var(--color-eve-border)]">
        ...
        >
          ...
          <div class="md:col-span-5 flex flex-col min-w-0">
            <span class="font-bold text-white group-hover:text-[var(--color-eve-accent)] transition-colors truncate">
              {k.shipName || k.ship}
            </span>
    ...
    {#if !$searchResults?.loading && $searchResults?.kills?.length === 0}
      <li class="p-8 text-center text-gray-500 text-sm tracking-widest uppercase">
